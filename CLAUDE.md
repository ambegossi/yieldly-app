...
- **Data**: APY history is fetched from the real DefiLlama `/chart/{poolId}` endpoint via `PoolRepo.findApyHistory`, cached with React Query (5-minute stale time). Mocks are used only in tests. (Original design proposed mock data; that approach was abandoned during implementation.)
